...
class MouthController(Node):
    def __init__(self):
        ...
        self.srv = self.create_service(Empty, 'audio_update', self.audio_update_callback)
    ...
    def audio_update_callback(self, request: Empty.Request, response: Empty.Response):
        try:
            if self.wav_file:
                self.wav_file.close()
