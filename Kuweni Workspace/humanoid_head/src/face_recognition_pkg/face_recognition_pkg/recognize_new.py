...
from ros2_ws.src.face_recognition_pkg.face_recognition_pkg.face_tracking.tracker.iou_tracker import BYTETracker
from face_tracking.tracker.visualize import plot_tracking

tracking_id_to_angle = {} #new

new_person_base = "/home/charu/Desktop/FYP/FACE RECOGNITION/datasets/new_persons"

# Device configuration
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# Face detector (choose one)
detector = SCRFD(model_file="/home/charu/Desktop/FYP/FACE RECOGNITION/face_detection/scrfd/weights/scrfd_2.5g_bnkps.onnx")
# detector = Yolov5Face(model_file="face_detection/yolov5_face/weights/yolov5n-face.pt")

# Face recognizer
recognizer = iresnet_inference(
    model_name="r100", path="/home/charu/Desktop/FYP/FACE RECOGNITION/face_recognition/arcface/weights/arcface_r100.pth", device=device
)
...
def get_new_person_folder(base_path):
    """Generate a new folder name like person_1, person_2, etc."""
    i = 1
    while True:
            ...
            for j in range(len(detection_bboxes)):
                ...
                if mapping_score > 0.9:
                    ...
                    if name is not None:
                        if score < 0.25:
                            caption = "UN_KNOWN"
                            # save the image in new_person folder
                            # call add_person script
                            # Get a new unique folder for the unknown person
                            new_folder_path = get_new_person_folder(new_person_base)
                            
                            # Save the aligned face image
                            save_path = os.path.join(new_folder_path, "face.jpg")
                            cv2.imwrite(save_path, face_alignment)
                            print(f"Saved unknown face to {save_path}")

                            parser = add_persons.argparse.ArgumentParser()
                            parser.add_argument(
                                "--backup-dir",
                                type=str,
                            ...
                            add_persons.add_persons(**vars(opt))
                    ...
                    #new
                    # Angle Estimation
                    x_min, y_min, x_max, y_max = tracking_bboxes[i]
                    bbox_center_x = (x_min + x_max) / 2
                    frame_width = raw_image.shape[1]
                    relative_x = (bbox_center_x / frame_width) - 0.5
                    angle = relative_x * 120  # 120° (i took it as 120 FOV)
                    tracking_id_to_angle[tracking_ids[i]] = angle

                    if frame_id % 10 == 0:

                        print("Current Tracking ID to Angle Mapping:")
                        for tid, angle in tracking_id_to_angle.items():
                            print(f"ID {tid}: {angle:.2f} degrees")
                        print("-" * 30)
...
def main():
    ...
    file_name = "/home/charu/Desktop/FYP/FACE RECOGNITION/face_tracking/config/config_tracking.yaml"
