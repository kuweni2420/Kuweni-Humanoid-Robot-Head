...
from face_recognition_pkg.face_recognition.arcface.utils import read_features
...
detector = SCRFD(model_file="/home/quanta/Desktop/Head-Code-Old/receptionist-3.0-head/build/face_recognition_pkg/face_recognition_pkg/face_detection/scrfd/weights/scrfd_2.5g_bnkps.onnx")
...
recognizer = iresnet_inference(
    model_name="r100", path="/home/quanta/Desktop/Head-Code-Old/receptionist-3.0-head/build/face_recognition_pkg/face_recognition_pkg/face_recognition/arcface/weights/arcface_r100.pth", device=device
)
...
def get_feature(face_image):
    """
    Extract facial features from an image using the face recognition model (NumPy-based preprocessing).
    """
    # Convert to RGB
    face_image = cv2.cvtColor(face_image, cv2.COLOR_BGR2RGB)

    # Resize to 112x112
    face_image = cv2.resize(face_image, (112, 112)).astype(np.float32)
    ...
    # Convert to tensor
    face_tensor = torch.tensor(face_image).unsqueeze(0).to(device)

    # Run through the recognition model
    emb_img_face = recognizer(face_tensor)[0].cpu().numpy()
...
def add_person(face_img, name_person, 
                faces_save_dir = "/home/kuweni/ssd/ros2_ws/src/face_recognition_pkg/face_recognition_pkg/datasets/data/", 
                features_path = "/home/kuweni/ssd/ros2_ws/src/face_recognition_pkg/face_recognition_pkg/datasets/face_features/feature"):

    images_emb =[]
    images_name = []
    
    # Create a folder in person name
    folder_path = os.path.join(faces_save_dir, name_person)
    os.makedirs(folder_path, exist_ok=True)

    # Include the image of the face
    number_files = len(os.listdir(folder_path))
    image_path = os.path.join(folder_path, f"img_{number_files}.jpg")        
    cv2.imwrite(image_path, face_img)

    # Get the feature embeddings
    images_emb.append(get_feature(face_image=face_img))
    images_name.append(name_person)
...
def edit(features_path = "/home/kuweni/ssd/ros2_ws/src/face_recognition_pkg/face_recognition_pkg/datasets/face_features/feature"):
# Extract the two arrays
    features = read_features(features_path)

    old_images_name, old_images_emb = features
    # Remove last two entries
    # image_names = np.delete(old_images_name,4)
    # image_embeddings = np.delete(old_images_emb,4)
    image_names = []
    image_embeddings = []

    # image_names.remove('person_1')
    # image_embeddings.pop(4)
    # Modify the third name
    # image_names[2] = 'Tharusha'

    # Save back to a new .npz file (or overwrite)
    np.savez_compressed(features_path, images_name=image_names, images_emb=image_embeddings)


    # print("Modified .npz saved as 'modified_file.npz'")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--backup-dir",
        type=str,
        default="/home/kuweni/ssd/ros2_ws/src/face_recognition_pkg/face_recognition_pkg/datasets/backup",
        help="Directory to save person data.",
    )
    parser.add_argument(
        "--add-persons-dir",
        type=str,
        default="/home/kuweni/ssd/ros2_ws/src/face_recognition_pkg/face_recognition_pkg/datasets/new_persons",
        help="Directory to add new persons.",
    )
    parser.add_argument(
        "--faces-save-dir",
        type=str,
        default="/home/kuweni/ssd/ros2_ws/src/face_recognition_pkg/face_recognition_pkg/datasets/data/",
        help="Directory to save faces.",
    )
    parser.add_argument(
        "--features-path",
        type=str,
        default="/home/kuweni/ssd/ros2_ws/src/face_recognition_pkg/face_recognition_pkg/datasets/face_features/feature",
