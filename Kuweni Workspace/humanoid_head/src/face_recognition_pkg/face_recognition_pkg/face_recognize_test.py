...
class FaceRecognitionNode(Node):
    def __init__(self):
        ...
        # Recognition frame queue (for async worker)
        ...
        self.recognition_worker_thread = threading.Thread(target=self.face_recognition_worker)

        # Start all threads
        self.display_thread.start()
        self.tracking_thread.start()
        self.recognition_thread.start()
        self.recognition_worker_thread.start()

        self.get_logger().info("FaceRecognitionNode started successfully.")
    ...
    # Get features of one face image
        ...
        face_image = (face_image / 255. - 0.5) / 0.5
        face_image = np.transpose(face_image, (2, 0, 1))
        face_image = torch.tensor(face_image).unsqueeze(0).to(self.device)
    ...
    def run_recognition(self):
        while rclpy.ok():
            if self.frame_id % 10 != 0:  # optional throttling
                continue
            if not self.data_mapping:
                continue
            try:
                self.recognition_queue.put_nowait(self.data_mapping.copy())
            except queue.Full:
                self.get_logger().warn("Recognition queue full; dropping frame.")
            time.sleep(0.005)

    def face_recognition_worker(self):
        while rclpy.ok():
            try:
                data = self.recognition_queue.get(timeout=1)
            except queue.Empty:
                continue
                ...
                for j, det_bbox in enumerate(detection_bboxes):
                    if self.mapping_bbox(track_bbox, det_bbox) > 0.9:
                        try:
                            ...
                            # Make sure you unpack only the first 4 elements
                            x1, y1, x2, y2 = map(int, detection_bboxes[j][:4])
                            h_pad = int(0.1*(y2-y1)); w_pad = int(0.1*(x2-x1))
                            x1 = max(0, x1-w_pad); y1 = max(0, y1-h_pad)
                            x2 = min(raw_image.shape[1], x2+w_pad); y2 = min(raw_image.shape[0], y2+h_pad)
                            aligned_face = raw_image[y1:y2, x1:x2]
                            if aligned_face.size == 0:       # guard-rail
                                continue

                            # Skip if crop is emptycd
                            if aligned_face is None or aligned_face.size == 0:
                                self.get_logger().warn("Empty face crop; skipping.")
                                continue
                        ...
                        caption = "UN_KNOWN" if name is not None and score < 0.4 else f"{name}:{score:.2f}"
            ...
            for usr in self.active_usrs.data:
                usr.missed_frames += 0 if usr.id in current_ids else 1
    ...
    def new_usr_callback(self,msg:People):
        ...
        id = msg.id
        name = msg.name
        self.get_logger().info(f"got name{name}")
        for usr in self.active_usrs.data:
            if usr.id == id:
                usr.name = name
                break
