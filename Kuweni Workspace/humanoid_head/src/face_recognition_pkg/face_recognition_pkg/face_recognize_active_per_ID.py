    ...
    def run_recognition(self):
        while rclpy.ok():
            ...
            for i, track_bbox in enumerate(tracking_bboxes):
                ...
                now = time.time()
                last_time = self.last_recognition_time.get(track_id, 0)

                # Run ArcFace only if cooldown has passed
                ...
                for j, det_bbox in enumerate(detection_bboxes):
                    if self.mapping_bbox(track_bbox, det_bbox) > 0.9:
                        ...
                        hor_angle = round(max(-60, min(60, ((bbox_center_x / frame_width) - 0.5) * -120)), 2)
                        ver_angle = round(max(-30, min(30, ((bbox_center_y / frame_height) - 0.5) * -60)), 2)
            ...
            # Remove users with too many missed frames
            ...
            # Clean up old timestamps from cooldown tracker
            for old_id in list(self.last_recognition_time):
                if old_id not in current_ids:
                    del self.last_recognition_time[old_id]
