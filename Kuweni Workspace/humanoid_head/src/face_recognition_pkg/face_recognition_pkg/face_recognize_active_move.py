...
from face_recognition_pkg.Testing.unknown_enrollment_logger import log_unknown_and_enrollment
# from face_recognition_pkg.Testing.recognition_latency_logger import log_start_time, log_end_time
...
class FaceRecognitionNode(Node):
    def __init__(self):
        ...
        self.grace_timeout_seconds = 3  # change as needed
        self.last_seen_time = {}  # track_id → last time they were seen
    ...
    def run_recognition(self):
        ...
        while rclpy.ok():
            ...
            for i, track_bbox in enumerate(tracking_bboxes):
                ...
                # Always compute angles
                x_min, y_min, x_max, y_max = track_bbox
                bbox_center_x = (x_min + x_max) / 2
                frame_width = raw_image.shape[1]
                hor_angle = max(-40, min(40, round(((bbox_center_x / frame_width) - 0.5) * -80, 2)))
                ...
                for j, det_bbox in enumerate(detection_bboxes):
                    if self.mapping_bbox(track_bbox, det_bbox) > 0.9:
                        ...
                        caption = f"Unknown_{track_id}" if name is not None and score < 0.4 else f"{name}:{score:.2f}"


                        if caption == "Unknown":
                            if track_id not in self.unknown_timestamps:
                                start_time = time.time()
                                self.unknown_timestamps[track_id] = start_time
                                log_unknown_and_enrollment(
                                    face_img=aligned_face,
                                    label=caption,
                                    was_unknown=True,
                                    unknown_id=track_id
                                )
                            # Update last seen frame or add new entry
                            self.unknown.append({
                                "id": track_id,
                                "angle": hor_angle,
                                "image": aligned_face
                            ...
                            self.get_logger().info(f"Detected new unknown: {caption}")
            ...
            for usr in self.active_usrs.data:
                if usr.id in current_ids:
                    self.last_seen_time[usr.id] = now


            # Only keep users seen within grace timeout or still visible
            self.active_usrs.data = [
                p for p in self.active_usrs.data
                if (p.id in current_ids or (p.id in self.last_seen_time and now - self.last_seen_time[p.id] < self.grace_timeout_seconds))
            ]

            # Remove stale entries from last_seen_time
            self.last_seen_time = {
                tid: ts for tid, ts in self.last_seen_time.items()
                if tid in [p.id for p in self.active_usrs.data]
            }

            current_state = set((p.id, self.id_face_mapping.get(p.id, "")) for p in self.active_usrs.data)
            # current_state = set((p.id, self.id_face_mapping.get(p.id, ""), p.likely_present) for p in self.active_usrs.data)
    ...
    def new_usr_callback(self,msg:People):
        ...
        for usr in self.unknown:
            if usr["id"] == id:
                ...
                start_time = self.unknown_timestamps.get(id)
                if start_time:
                    end_time = time.time()
                    enrollment_time = end_time - start_time
                    label_change_time = enrollment_time  # same in this context
                    log_unknown_and_enrollment(
                        face_img=img,
                        label=name,
                        was_unknown=False,
                        ...
                        label_change_time=label_change_time
                    )
                    del self.unknown_timestamps[id]
                self.get_logger().info(f"[Enrollment] Added new person: {name} (ID: {id})")
